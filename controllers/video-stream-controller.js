...
import { getFile as _getFile } from "../db/handle-db";

const getFile = (file_name, callback) => {
  readFile(resolve(process.env.FILE_UPLOAD_PATH, file_name), callback);
};
...
export function renderVideo(req, res) {
  const fileDetails = _getFile(req.params.id);
  if (!fileDetails) {
